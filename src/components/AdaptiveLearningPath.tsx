    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900">
              Adaptive Learning Path: <span className="text-indigo-600">{activeChild.name}</span>
            </h1>
        ...
        <div className="flex items-center gap-3 self-start md:self-auto shrink-0">
          <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 text-right">
            <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-wider block">Child XP & Level</span>
            <span className="text-sm font-bold text-indigo-900 flex items-center justify-end gap-1.5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          ...
          <span className="text-[10px] text-emerald-600 font-semibold">85%+ score retention</span>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          ...
          <span className="text-[10px] text-indigo-600 font-semibold">Dynamic sprints queued</span>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
          ...
          <p className="text-2xl font-bold text-purple-600">{hotsCount}</p>
          <span className="text-[10px] text-purple-600 font-semibold">Olympiad & IIT track</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 sm:p-4 flex flex-wrap items-center justify-between gap-3 shadow-xs">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-2.5 py-1 rounded-lg border border-slate-300 text-xs bg-white text-slate-800 focus:outline-hidden focus:ring-1 focus:ring-indigo-500"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-3">
          <h2 className="text-sm font-bold text-slate-800 flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400 font-normal">Click a node to view RAG diagnostic analysis</span>
          </h2>

          {filteredNodes.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center">
              <Compass className="w-8 h-8 text-slate-300 mx-auto mb-2" />
              <p className="text-xs font-semibold text-slate-700">No topic nodes match the current filter</p>
              <p className="text-[11px] text-slate-400 mt-1">Try resetting the subject or track filters above.</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center justify-between gap-4 pt-2 border-t border-slate-100">
                    <div className="flex-1 max-w-[200px]">
                      <div className="flex justify-between text-[11px] mb-1">
                        ...
                        <span className="font-bold text-slate-800">{node.masteryPercentage}%</span>
                      </div>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold flex items-center gap-1.5 shadow-xs transition-colors shrink-0"
                    >
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-xs sticky top-20 space-y-5">
              <div className="flex items-start justify-between gap-2 pb-3 border-b border-slate-100">
                <div>
                  ...
                  <h3 className="font-bold text-base text-slate-900 mt-0.5">{selectedNode.topic}</h3>
                  <p className="text-xs text-slate-400">{selectedNode.chapterName} • {selectedNode.classGrade} ({selectedNode.board})</p>
                </div>

                <div className="text-right shrink-0">
                  <span className="text-2xl font-bold text-indigo-600">{selectedNode.masteryPercentage}%</span>
                  <span className="text-[10px] text-slate-400 block">K-Graph Score</span>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {selectedNode.curatedResources.map((res, idx) => (
                    <a
                      ...
                      className="block p-2.5 bg-slate-50 hover:bg-indigo-50/50 rounded-xl border border-slate-200 hover:border-indigo-200 transition-colors group"
                    >
                      <div className="flex items-center justify-between text-xs font-bold text-slate-900 group-hover:text-indigo-700 mb-0.5">
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-xs transition-all"
                >
